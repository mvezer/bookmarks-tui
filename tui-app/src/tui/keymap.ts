...
const BINDINGS_ACTIONS_HELP: Record<keyof typeof KeymapEvents, string> = {
  ...
  [KeymapEvents.editBookmark]: 'Edits the selected bookmark (external editor)',
  [KeymapEvents.newBookmark]: 'Creates a new bookmark (external editor)',
...
export class Keymap extends EventEmitter {
  ...
  static init(
    ...
    if (!Keymap._instance) {
      Keymap._instance = new Keymap(renderer, keymapDefintions);
    }
